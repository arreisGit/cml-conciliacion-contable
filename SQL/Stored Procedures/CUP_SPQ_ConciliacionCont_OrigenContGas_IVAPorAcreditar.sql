...
IF EXISTS (SELECT * 
		   FROM SYSOBJECTS 
		   WHERE ID = OBJECT_ID('dbo.CUP_SPQ_ConciliacionCont_OrigenContGas_IVAPorAcreditar') AND 
				 TYPE = 'P')
BEGIN
  DROP PROCEDURE dbo.CUP_SPQ_ConciliacionCont_OrigenContGas_IVAPorAcreditar 
END	
...
/* =============================================
  ...
  de Gastos, con la suficiente iformacion para
  poderlos cruzar "lado a lado" con su póliza contable.
 
  Example: EXEC CUP_SPQ_ConciliacionCont_OrigenContGas_IVAPorAcreditar 63527, 1, 2017, 1
 ============================================= */

CREATE PROCEDURE dbo.CUP_SPQ_ConciliacionCont_OrigenContGas_IVAPorAcreditar
  @Empleado   INT,
  @Tipo      INT,
  @Ejercicio INT,
  @Periodo   INT
AS BEGIN 
  ...
  JOIN Gasto m ON origenCont.Mov = m.Mov
  JOIN Prov ON Prov.Proveedor = m.Acreedor
