...
AS BEGIN 
  ...
  LEFT JOIN CUP_ConciliacionCont_Excepciones cEx ON cEx.TipoConciliacion = 3 
                                                AND cEx.TipoExcepcion = 1 
                                                AND cEx.Valor = a.Cuenta
  ...
  AND a.Periodo = @Periodo
  AND cEx.ID IS NULL -- Excepciones Cta
  ...
  -- Reevaluaciones de Movimientos del mes
    ...
    LEFT JOIN CUP_ConciliacionCont_Excepciones eX ON ex.TipoConciliacion = @Tipo
                                                  ...
                                                  AND ex.Valor = c.Cliente
    WHERE 
      t.Clave = 'CXC.RE'
    AND c.Estatus = 'CONCLUIDO'
    AND c.Ejercicio = @Ejercicio
    AND c.Periodo = @Periodo
    AND eX.ID IS NULL -- Excepciones Cta
END